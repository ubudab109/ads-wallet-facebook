...
@section('subcontent')
      ...
      <div class="modal-dialog modal-lg">
          <div class="modal-content">
              <div class="modal-body p-10">
                <form action="{{route('addBankAdmin')}}" method="POST">
                  @csrf
                  <div class="preview">
                    <div class="mt-5 mb-5">
    ...
    {{-- DETAIL FOR EDIT BANK MODAL --}}
    <div id="editBank" class="modal" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog modal-lg">
          <div class="modal-content">
              <div class="modal-body p-10">
                <div class="preview">
                  ...
                  <div class="mt-5 mb-5">
                    <label for="">Nama Pada Bank </label>
                    <input type="text" class="form-control" id="account_holder_bank">
                  </div>
                  <div class="mt-5 mb-5" id="btn_edit">
...
@section('script')
    <script>
      ...
      var table = new Tabulator("#list-bank", {
        ajaxURL:"{{route('listBank')}}", //set url for ajax request
        ajaxFiltering: true,
        ajaxSorting: true,
        printAsHtml: true,
        printStyled: true,
                                                ...
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu w-48">
                                                        <div class="dropdown-menu__content box dark:bg-dark-1 p-2">
                                                          <a href="javascript:;" onclick="editBank('${cell.getData().id}'); return false;" class="flex items-center block p-2 transition duration-300 ease-in-out bg-white dark:bg-dark-1 hover:bg-gray-200 dark:hover:bg-dark-2 rounded-md" data-toggle="modal" data-target="#editBank">
                                                              Edit
                                                          </a>
      ...
      {
        var url = "{{route('detailBankAdmin', ':id')}}";
        ...
        $.ajax({
          ...
          success: function(res) {
            ...
            $("#account_holder_bank").val(res.account_holder_bank);
            $("#btn_edit").html(`
                    <button type="button" onclick="updateBank('${id}')" class="btn btn-primary w-24 mr-1 mb-2">Submit</button>
                    <a id="programmatically-hide-modal-edit" onclick="closeModalEdit()" class="btn btn-secondary mr-1">Cancel</a>
            `)
          }
        })
      }
      ...
      {
        var url = "{{route('updateBankAdmin', ':id')}}";
        url = url.replace(':id', id);
        var bank_name = $("#bank_name").val();
        var bank_number = $("#bank_number").val();
        var account_holder_bank =  $("#account_holder_bank").val();
        var formData = new FormData();

        formData.append('bank_name', bank_name);
        formData.append('bank_number', bank_number);
        formData.append('account_holder_bank', account_holder_bank);
        $.ajax({
          headers: {
          ...
          error: function (err) { 
            if (err.status == 422) {
              ...
              alert('Terjadi Kesalahan. Mohon Diulangi');
            }
          }
        });
      }
      ...
      {
        var url = "{{route('deleteBankAdmin', ':id')}}";
        url = url.replace(':id', id);
        if (confirm('Anda Yakin Ingin Menghapus Data Bank Ini?. Bank yang sudah terpakai untuk transaksi tidak akan bisa dihapus')) {
          $.ajax({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            ...
            error: function (err) {
              alert('Terjadi Kesalahan atau Bank sudah dipakai user untuk transaksi. Mohon Diperiksa kembali');
            }
          });
        }
      }
